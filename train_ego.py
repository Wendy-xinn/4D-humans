...
from hmr2.configs import dataset_config, CACHE_DIR_4DHUMANS
...
from train_pimu_3DPW import HMR2DataModule_ego
...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    datamodule = HMR2DataModule_ego(cfg, dataset_cfg)

    # Setup model
    model = HMR2(cfg)

    checkpoint_dir = './logs/train/runs/ego/checkpoints'
    # 🔑 关键：区分两种场景
    if cfg.MODEL.BACKBONE.RESUME_FROM_CHECKPOINT:
        # 🔄 场景A: 恢复训练（从 last.ckpt 继续）
        # 让 Lightning 自动处理，但需确保 model.on_load_checkpoint 已重写
        ckpt_path = os.path.join(checkpoint_dir, 'last-v4.ckpt')  # 或具体路径
        # ckpt_path = 'last-v4.ckpt'
        model.cfg.MODEL.BACKBONE.load_pretrained = False
        log.info(f"Resuming training from {ckpt_path}")
    else:
        # 🚀 场景B: 首次训练（从预训练 backbone 开始）
        # __init__ 中已手动加载预训练权重，这里禁用自动恢复
        ckpt_path = None
        model.cfg.MODEL.BACKBONE.load_pretrained = True
