...
class SMPLTransformerDecoderHead(nn.Module):
    ...
    def __init__(self, cfg, is_ego='false'):
        super().__init__()
        self.cfg = cfg
        # 是否为 Ego 模式，默认为 False
        self.is_ego = is_ego
        ...
        if self.is_ego:
            init_body_pose = init_body_pose.clone()
            rot_180_mat = R.from_euler('y', 180, degrees=True).as_matrix().astype(np.float32)
            if self.joint_rep_type == '6d':
                # 提取前两列 (R1, R2)
                raw_6d = init_body_pose[0, :6].cpu().numpy().reshape(3, 2)
                # 通过外积恢复第三列 R3，确保正交性
                col1, col2 = raw_6d[:, 0], raw_6d[:, 1]
                col3 = np.cross(col1, col2)
                rot_mat_old = np.stack([col1, col2, col3], axis=1)
                # 叠加旋转：R_new = R_180 @ R_old
                rot_mat_new = rot_180_mat @ rot_mat_old  
                # 重新转回 6D (前两列展平)
                new_6d = rot_mat_new[:, :2].flatten() # [6]
                init_body_pose[0, :6] = torch.from_numpy(new_6d)
            elif self.joint_rep_type == 'aa':
                # 提取前 3 维 global_orient
                raw_aa = init_body_pose[0, :3].cpu().numpy()
                # 轴角转矩阵
                rot_mat_old = R.from_rotvec(raw_aa).as_matrix()
                # 叠加旋转
                rot_mat_new = rot_180_mat @ rot_mat_old
                # 矩阵转回轴角
                new_aa = R.from_matrix(rot_mat_new).as_rotvec() # [3]
                init_body_pose[0, :3] = torch.from_numpy(new_aa)
            init_cam = torch.tensor([[1.0, 0.0, 0.2]]) 
        else:
